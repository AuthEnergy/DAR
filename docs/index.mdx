...
icon: database
---
<sub>First published: 11 March 2026 · Last updated: 26 March 2026 · Status: `PROPOSED`</sub>
<Warning>
  **This is a design proposal for review and not yet an operational production service.**<p/>
  If you wish to discuss the design with the author, please contact us at [contact@auth.energy](mailto:contact@auth.energy)<p/>
  Visit the [Github repo](https://github.com/AuthEnergy/dar) to further documentation and a fully working, downloadable proof of concept (but there's likely many bugs). Get in touch if you'd like to contribute.
</Warning>

The register is not a [Consumer Consent Solution](https://retailenergycode.co.uk/our-programmes/consumer-consent-solution/) exclusively. It's far more than this, it records access under any lawful basis — consent, legitimate interests, public task, legal obligation, or contract for any type of customer (not just Consumers). The Central Register holds the registration only; it does not validate or enforce the legal basis claimed by the controller (that's what GDPR is for).

<Info>
  This is a practical, lightweight alternative approach energy data transparency. It:
...
- supports a display of access records in [Central Customer Portal](/data-access-register/customer-portal) and/or any customer-facing user service (although this would allow users to see who else the customer has consented to)
- supports other trusted portal operators (Citizens Advice could host their own branded customer portal)
- fully SEC compliant & compatible (no regulatory change required)
...
- supports separate or combined controllers/users + sole/joint/share controllers
- could be enhanced to promote other services to customers
- historic and expired access can be added to the system at any time (to complete historic view of consent)
...
See the [Identity Records guide](/data-access-register/identity-guide) for full details including passkey enrolment and credential management.
