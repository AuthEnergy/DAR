...
sidebarTitle: Onward Sharing Framework
...
```mermaid
graph TD
    subgraph Z1["Zone 1 — SEC parties · SEC + PCF + UK GDPR"]
        A["DCC · Suppliers · DNOs · Other Users. e.g. Octopus, National Grid, n3rgy, Hildebrand"]
    end

    subgraph Z2["Zone 2 — Contractual extension · MDA + UK GDPR"]
        C["App providers · Aggregators · Sub-processors. e.g. Mixergy, uSwitch, Loop"]
    end

    subgraph Z3["Zone 3 — Beyond sector reach · UK GDPR only"]
        E["Independent controllers · Anonymised data · Derived datasets. e.g. price comparison sites, research bodies, ad-tech"]
    end

    Z1 -->|"MDA required<br/>before disclosure"| Z2
    Z2 -->|"GDPR only<br/>no sector reach"| Z3
