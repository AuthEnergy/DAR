...
### Step 1: Check Existence and Available Methods
...
No `ir` key or PII is returned. The response indicates both whether a record exists and which re-identification methods are available:

```json
{
  "exists": true,
  "mpxn": "1234567890123",
  "available-methods": ["passkey-assert", "magic-link", "passkey-register"]
}
```

| Method in `available-methods` | Meaning |
|---|---|
| `passkey-assert` | Customer has a registered passkey — preferred method |
| `magic-link` | An email address is stored — can dispatch a link |
| `passkey-register` | Always present — a new passkey can always be enrolled |

Data User B uses this to decide which method to offer the customer before initiating the challenge. If `exists: false`, Data User B should create a new Identity Record via the standard flow.
